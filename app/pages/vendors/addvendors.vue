<template>
  <div>
    <div class="pt-2 mb-6">
      <header>
        <h1 class="text-2xl font-bold rounded-md p-2 bg-[#5E7AC4] text-white">Add Vendor</h1>
      </header>
    </div>
    ...
    <form @submit.prevent="addVendor">
      <div class="grid gap-6 mb-0 md:grid-cols-2">
        <div>
          ...
          <p v-if="nameExists" class="text-red-500 text-sm mt-1">A vendor with this name already exists.</p>
        </div>
        <div>
...
const { data: vendors, refresh } = await useAsyncData('vendors', async () => {
